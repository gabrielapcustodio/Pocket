...
export const realizaCompra = (idBotao, valorTela, economDigitada, tipoTitulo, id) => {
  const botao = document.getElementById(idBotao);
  if (botao) {
    botao.addEventListener("click", function (event) {
      event.preventDefault();
      let valorDaCompra = botao.innerHTML;
...
function atualizaValor(valorTela, valorDaCompra) {
  if (checaVazio("valorCarteira") === true) {
    let novoValor = valorTela.innerHTML -= valorDaCompra;
    return novoValor;
  }
}
